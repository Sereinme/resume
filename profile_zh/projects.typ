...
#cv-entry(
  ...
  location: [重庆],
  description: list(
    [针对机器人在未知环境中自主导航的需求，研究基于 VLM 的具身智能导航技术并构建场景知识图谱 (SKG)，提升复杂环境下的路径规划能力],
    [设计并实现多模态特征融合策略，将视觉感知与语言理解相耦合，使机器人/智能体无需先验地图即可完成自主导航与任务执行],
  ),
...
#cv-entry(
  ...
  location: [重庆],
  description: list(
    [剖析 APA 感知模块的定位误差来源，建立误差在控制系统中的传递模型，定位精度损失的根因],
    [采用 ESKF 进行多传感器融合定位，结合图优化提升位姿估计精度，为泊车路径规划提供更可靠的定位基础],
  ),
...
#cv-entry(
  ...
  location: [北京],
  description: list(
    [基于 Verilog 在 FPGA 平台上构建高效流式数据压缩架构],
...
#cv-entry(
  ...
  description: list(
    ...
    [构建人脸图像的子空间模型，探索不同光照、表情、遮挡条件下的鲁棒特征提取与分类策略],
  ),
...
#cv-entry(
  ...
  location: [北京],
  description: list(
    [面向音频与视频双模态信号的联合识别与转录问题，实现多模态信息融合],
...
#cv-entry(
  ...
  location: [北京],
  description: list(
    [模拟鸡尾酒会效应中的目标语音分离与辨识场景，研究复杂声学环境下的说话人识别问题],
